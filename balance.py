...
data=data.sort(['SEASON','GAME_CODE','TEAM_ID'])
grouped=data.groupby(['SEASON','GAME_CODE','TEAM_ID'])
data['cum_points']=grouped.POINTS.apply(accum)
data=data.sort(['SEASON','GAME_CODE','event_time'])

#to get separate cum_points for starter and followers
game_gr=data.groupby(['SEASON','GAME_CODE'])
...
new_data=new_data.sort(['SEASON','GAME_CODE','event_time']).reset_index(drop=True)

grouped=new_data.groupby(['SEASON','GAME_CODE'])
...
group_sft=new_data.groupby(['SEASON','GAME_CODE'])
...
new_data.to_csv(path+'/'+dir+'_all_lead.csv')
...
lead_prob.to_csv(path+'/'+dir+'_probScoreGivenLead.csv')
