...
data=pandas.DataFrame.from_csv(folder+file, \
	parse_dates=False)
...
for name, group in grouped:#for each game
	...
	winner=0
	crossed=0
	for i in reversed(xrange(num_events)): #b/c goes to next event
...
leadUnsafeA=LeadA[:,1:]*(inLeadA[:,1:]==0)#get lead where it's unsafe
...
scount=0
ucount=0
for i in xrange(games): #!!!!This ignores the losing team in leadUnsafe!!!
...
#mt_test=t_test.mean()
#sdt_test=t_test.std()
#tz_test=(t_test-mt_test)/sdt_test
#X_test=np.vstack([t_test,lead_test]).T
...
#test[:,2:4]=Xz_test
#test[:,4]=y_test
#train.tofile('timeLeadSafeTrainAll.csv',sep=',')
#test.tofile('timeLeadSafeTestAll.csv',sep=',')